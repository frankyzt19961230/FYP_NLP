...
def outfileGen():
    csvDir = "/Users/frank/PycharmProjects/FYP_classification/Dataset/paper/csv/"
    fileoutput ='/Users/frank/PycharmProjects/FYP_classification/Dataset/paper/paper_outfile.csv'
    os.remove(fileoutput)
    for fileinput in os.listdir(csvDir):
        with open(csvDir+fileinput) as f:
            reader = csv.reader((line.replace('\0','') for line in f))
            #print(list(reader))
            title = fileinput.strip('/Users/frank/PycharmProjects/FYP_classification/Dataset/paper/csv/.')
            content= ''
            for row in reader:
                #print(row)
...
def main():
    ...
    vectorizer_set = {
        'Tdidf_diagram': TfidfVectorizer(sublinear_tf=True, min_df=5, norm='l2', decode_error="ignore",encoding='latin-1', ngram_range=(1,2), stop_words='english'),
    ...
    # Preprocess
    outfileGen()
    outfile = "/Users/frank/PycharmProjects/FYP_classification/Dataset/paper/paper_outfile.csv"
    #TSNE(outfile)

    # Origin CSV insert to database
    Originfile_insert(outfile, collection_set['Paper'])
    # keywords
    feature_keywords(outfile,collection_set['Paper'],vectorizer_set['Tdidf_diagram'])
